...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        /// Gets tangent vector on path based on 'time' (where 0 is start, and 1 is end of path).
        ...
        public float GetTAfterXUnitsFromT(float t, float units, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
            ...
            return GetClosestTimeOnPath(GetPointAtDistance((arcDistanceOfSegment * data.percentBetweenIndices) + units + cumulativeLengthAtEachVertex[data.previousIndex]));
        }
